...
func (enc *Encoder) encodeSideInfo() {
	...
	bs := &enc.bitstream
	bs.putBits(2047, 11)
	bs.putBits(uint32(enc.Mpeg.Version), 2)
	bs.putBits(uint32(enc.Mpeg.Layer), 2)
	if enc.Mpeg.Crc == 0 {
		bs.putBits(uint32(1), 1)
	} else {
		bs.putBits(uint32(0), 1)
	}
	bs.putBits(uint32(enc.Mpeg.BitrateIndex), 4)
	bs.putBits(uint32(enc.Mpeg.SampleRateIndex%3), 2)
	bs.putBits(uint32(enc.Mpeg.Padding), 1)
	bs.putBits(uint32(enc.Mpeg.Ext), 1)
	bs.putBits(uint32(enc.Mpeg.Mode), 2)
	bs.putBits(uint32(enc.Mpeg.ModeExt), 2)
	bs.putBits(uint32(enc.Mpeg.Copyright), 1)
	bs.putBits(uint32(enc.Mpeg.Original), 1)
	bs.putBits(uint32(enc.Mpeg.Emphasis), 2)
	if enc.Mpeg.Version == MPEG_I {
		bs.putBits(uint32(enc.mainDataBegin), 9)
		if enc.Wave.Channels == 2 {
			bs.putBits(uint32(sideInfo.PrivateBits), 3)
		} else {
			bs.putBits(uint32(sideInfo.PrivateBits), 5)
		}
	} else {
		bs.putBits(uint32(enc.mainDataBegin), 8)
		if enc.Wave.Channels == 2 {
			bs.putBits(uint32(sideInfo.PrivateBits), 2)
		} else {
			bs.putBits(uint32(sideInfo.PrivateBits), 1)
		}
	}
	if enc.Mpeg.Version == MPEG_I {
		for ch := int64(0); ch < enc.Wave.Channels; ch++ {
			for scaleFactorSelectionInfoBand := 0; scaleFactorSelectionInfoBand < 4; scaleFactorSelectionInfoBand++ {
				bs.putBits(uint32(sideInfo.ScaleFactorSelectInfo[ch][scaleFactorSelectionInfoBand]), 1)
			}
		}
	}
	for gr := int64(0); gr < enc.Mpeg.GranulesPerFrame; gr++ {
		for ch := int64(0); ch < enc.Wave.Channels; ch++ {
			granInfo := &sideInfo.Granules[gr].Channels[ch].Tt
			bs.putBits(uint32(granInfo.Part2_3Length), 12)
			bs.putBits(uint32(granInfo.BigValues), 9)
			bs.putBits(uint32(granInfo.GlobalGain), 8)
			if enc.Mpeg.Version == MPEG_I {
				bs.putBits(uint32(granInfo.ScaleFactorCompress), 4)
			} else {
				bs.putBits(uint32(granInfo.ScaleFactorCompress), 9)
			}
			bs.putBits(0, 1)
			for region := 0; region < 3; region++ {
				bs.putBits(uint32(granInfo.TableSelect[region]), 5)
			}
			bs.putBits(uint32(granInfo.Region0Count), 4)
			bs.putBits(uint32(granInfo.Region1Count), 3)
			if enc.Mpeg.Version == MPEG_I {
				bs.putBits(uint32(granInfo.PreFlag), 1)
			}
			bs.putBits(uint32(granInfo.ScaleFactorScale), 1)
			bs.putBits(uint32(granInfo.Count1TableSelect), 1)
		}
	}
}
func (enc *Encoder) huffmanCodeBits(ix *[GRANULE_SIZE]int64, gi *GranuleInfo) {
	...
	paddingBits := int64(gi.Part2_3Length) - int64(gi.Part2Length) - bits
	if paddingBits > 0 {
		var (
			stuffingWords int64 = paddingBits / 32
			remainingBits int64 = paddingBits % 32
		)
