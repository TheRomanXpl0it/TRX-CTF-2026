	...
	for ch := enc.Wave.Channels - 1; ch >= 0; ch-- {
		for gr := int64(0); gr < enc.Mpeg.GranulesPerFrame; gr++ {
			...
			for band := 0; band < 32; band++ {
				...
				if band != 0 {
					...
					mdctEnc[band*18+0] += enc.Rng.Int31n(2*enc.coefRange[ch][gr][band*18+0]+1) - enc.coefRange[ch][gr][band*18+0]

					mdctEnc[band*18+1], mdctEnc[(band-1)*18+17-1] = cmuls(
						&mdctEnc[band*18+1], &mdctEnc[(band-1)*18+17-1],
						&MDCT_CS1, &MDCT_CA1,
					)
					mdctEnc[band*18+1] += enc.Rng.Int31n(2*enc.coefRange[ch][gr][band*18+1]+1) - enc.coefRange[ch][gr][band*18+1]

					mdctEnc[band*18+2], mdctEnc[(band-1)*18+17-2] = cmuls(
						&mdctEnc[band*18+2], &mdctEnc[(band-1)*18+17-2],
						&MDCT_CS2, &MDCT_CA2,
					)
					mdctEnc[band*18+2] += enc.Rng.Int31n(2*enc.coefRange[ch][gr][band*18+2]+1) - enc.coefRange[ch][gr][band*18+2]

					mdctEnc[band*18+3], mdctEnc[(band-1)*18+17-3] = cmuls(
						&mdctEnc[band*18+3], &mdctEnc[(band-1)*18+17-3],
						&MDCT_CS3, &MDCT_CA3,
					)
					mdctEnc[band*18+3] += enc.Rng.Int31n(2*enc.coefRange[ch][gr][band*18+3]+1) - enc.coefRange[ch][gr][band*18+3]

					mdctEnc[band*18+4], mdctEnc[(band-1)*18+17-4] = cmuls(
						&mdctEnc[band*18+4], &mdctEnc[(band-1)*18+17-4],
						&MDCT_CS4, &MDCT_CA4,
					)
					mdctEnc[band*18+4] += enc.Rng.Int31n(2*enc.coefRange[ch][gr][band*18+4]+1) - enc.coefRange[ch][gr][band*18+4]

					mdctEnc[band*18+5], mdctEnc[(band-1)*18+17-5] = cmuls(
						&mdctEnc[band*18+5], &mdctEnc[(band-1)*18+17-5],
						&MDCT_CS5, &MDCT_CA5,
					)
					mdctEnc[band*18+5] += enc.Rng.Int31n(2*enc.coefRange[ch][gr][band*18+5]+1) - enc.coefRange[ch][gr][band*18+5]

					mdctEnc[band*18+6], mdctEnc[(band-1)*18+17-6] = cmuls(
						&mdctEnc[band*18+6], &mdctEnc[(band-1)*18+17-6],
						&MDCT_CS6, &MDCT_CA6,
					)
					mdctEnc[band*18+6] += enc.Rng.Int31n(2*enc.coefRange[ch][gr][band*18+6]+1) - enc.coefRange[ch][gr][band*18+6]

					mdctEnc[band*18+7], mdctEnc[(band-1)*18+17-7] = cmuls(
						&mdctEnc[band*18+7], &mdctEnc[(band-1)*18+17-7],
						&MDCT_CS7, &MDCT_CA7,
					)
					mdctEnc[band*18+7] += enc.Rng.Int31n(2*enc.coefRange[ch][gr][band*18+7]+1) - enc.coefRange[ch][gr][band*18+7]
				}
			}
		}
		...
		for gr := int64(0); gr < enc.Mpeg.GranulesPerFrame; gr++ {
			for i := 0; i < GRANULE_SIZE; i++ {
				enc.S = enc.S*131 + int64(enc.mdctFrequency[ch][gr][i])
			}
		}
	}
}
